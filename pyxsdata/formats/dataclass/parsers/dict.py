...
class DictDecoder:
    ...
    def detect_type(self, data: dict | list) -> type[T]:
        ...
        clazz: type[T] | None = cast(
            type[T] | None, self.context.find_type_by_fields(set(keys))
        )
    ...
    def bind_dataclass(self, data: dict, clazz: type[T]) -> T:
        ...
        try:
            return cast(T, self.config.class_factory(clazz, params))
    ...
    def bind_derived_dataclass(self, data: dict, clazz: type[T]) -> Any:
        ...
        if clazz is generic:
            ...
            if xsi_type:
                real_clazz = cast(type[T] | None, self.context.find_type(xsi_type))
