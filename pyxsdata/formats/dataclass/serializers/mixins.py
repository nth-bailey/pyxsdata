...
from pyxsdata.exceptions import SerializerError, XmlWriterError
from pyxsdata.formats.converter import converter
from pyxsdata.formats.dataclass.context import XmlContext
from pyxsdata.formats.dataclass.models.elements import XmlMeta, XmlVar
...
type StartEvent = tuple[Literal["start"], str]
type AttrEvent = tuple[Literal["attr"], str, Any]
type DataEvent = tuple[Literal["data"], str]
type EndEvent = tuple[Literal["end"], str]
...
class EventHandler(abc.ABC):
    ...
    def write(self, events: EventIterator) -> None:
        ...
        for event in events:
            match event:
                case (XmlWriterEvent.START, tag):
                    self.start_tag(tag)
                case (XmlWriterEvent.END, tag):
                    self.end_tag(tag)
                case (XmlWriterEvent.ATTR, qname, value):
                    self.add_attribute(qname, value)
                case (XmlWriterEvent.DATA, data):
                    self.set_data(data)
                case _:
                    raise XmlWriterError(f"Unhandled event: `{event[0]}`")
