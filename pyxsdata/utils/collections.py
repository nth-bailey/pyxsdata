...
def unique_sequence[T](items: Iterable[T], key: str | None = None) -> list[T]:
...
def remove[T](items: Iterable[T], predicate: Callable) -> list[T]:
    """Return a new list without the items that match the predicate."""
    return [x for x in items if not predicate(x)]


def group_by[T](items: Iterable[T], key: Callable) -> dict[Any, list[T]]:
...
def first[T](items: Iterator[T]) -> T | None:
